...
void CBuilding::SurvivalReset()
{
	if (m_Type == BUILDING_REACTOR_DESTROYED)
	{
		...
		{
			new CBuilding(&GameServer()->m_World, m_Pos+vec2(0, 50), BUILDING_REACTOR, TEAM_BLUE);
...
void CBuilding::Move()
{
	...
	vec2 OldVel = m_Vel;
	GameServer()->Collision()->MoveBox(&m_Pos, &m_Vel, m_BoxSize, m_Bounciness);
...
void CBuilding::CreateLightningWallTop()
{
	...
	if (GameServer()->Collision()->IntersectLine(m_Pos, To, 0x0, &To))
	{
		...
		new CBuilding(&GameServer()->m_World, To+vec2(0, 15), BUILDING_LIGHTNINGWALL2, TEAM_BLUE);
	}
	else
		GameServer()->m_World.DestroyEntity(this);
}
...
void CBuilding::TakeDamage(int Damage, int Owner, int Weapon, vec2 Force)
{
	...
	if ((m_Type == BUILDING_TURRET || m_Type == BUILDING_TESLACOIL) && m_Team >= 0)
	{
...
{
	GameServer()->CreateExplosion(m_Pos, m_DamageOwner, GetBuildingWeapon(m_Type), false);
	...
	else if (m_Type == BUILDING_REACTOR)
	{
		if (!m_DestructionTriggered)
		{
			new CBuilding(&GameServer()->m_World, m_Pos, BUILDING_REACTOR_DESTROYED, TEAM_BLUE);
	...
	else if (m_Type == BUILDING_LIGHTNINGWALL)
	{
		...
		GameServer()->CreateExplosion(m_Pos, m_DamageOwner, WEAPON_NINJA, true);
		GameServer()->m_World.DestroyEntity(this);
	}
